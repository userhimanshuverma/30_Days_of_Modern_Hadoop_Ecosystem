#!/bin/bash
# Day 20: Apache Hive and Metastore Bootstrap script
# Location: Day-20-Hive-Architecture-HMS/docker/bootstrap.sh
...
copy_configs() {
  if [ -d "/workspace/configs" ]; then
    ...
    cp -r /workspace/configs/* /opt/hadoop/etc/hadoop/
    cp -r /workspace/configs/* /opt/hive/conf/
  fi
}
